...
%     env = py.reinforcement_learning_client_server.FTTPowerEnvironment(1, 2);
    ...
    client = py.ray.rllib.env.policy_client.PolicyClient('http://127.0.0.1:9911');
...
%         writematrix(output.LCOE,'LCOE.csv')
        ...
        if mod(num_of_runs_so_far, 1000) == 0
            save(sprintf('data/outputs/output_7-5-year-investments-%f.mat', floor(num_of_runs_so_far)), 'output')
