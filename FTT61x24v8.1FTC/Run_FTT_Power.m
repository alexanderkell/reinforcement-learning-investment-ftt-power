...
%         G_cum = sum(sum(sum(output.G)));
        G_cum = nanmean(output.G, "all");
%         U_cum = sum(sum(sum(output.U)));
        U_cum = nanmean(output.U, "all");
        E_cum = nanmean(output.E,'all');
%         CF_cum = sum(sum(sum(output.CF)));
        CF_cum = nanmean(output.CF, 'all');
...
%         W_cum = sum(sum(sum(output.W)));
        W_cum = nanmean(output.W, 'all');
%         I_cum = sum(sum(sum(output.I)));
        I_cum = nanmean(output.I, 'all');
        P_cum = nanmean(output.P,'all');
%         Fcosts_cum = sum(sum(sum(output.FCosts)));
        Fcosts_cum = nanmean(output.FCosts, 'all');
%         CO2_costs_cum = sum(sum(sum(output.CO2Costs)));
        CO2_costs_cum = nanmean(output.CO2Costs, 'all');
        % S_lim_cum = 
        % S_lim2_cum = 
