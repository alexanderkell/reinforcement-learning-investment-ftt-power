...
% for k = [7, 15] % Ireland and UK only
    ...
    writematrix(permute(RegSheet([5:28]+27*(k-1),58:58+EndYear-2013-1),[1 3 2]),'regsheet.csv')
    writematrix(REG(:,k,1:4:end),'REG.csv')
...
%     REG(:,k,4:4:end) = permute(RegSheet([5:28]+27*(k-1),58:58+EndYear-2013-1),[1 3 2]);
    REG(:,k,1:end) = permute(RegSheet([5:28]+27*(k-1),58:58+EndYear-2013-1),[1 3 2]);
    %Exogenous capacity
...
%     CarbP(1,k,4:4:end) = permute(CO2PSheet(67+k,15:15+EndYear-2013-1),[1 3 2]);
    CarbP(1,k,1:end) = permute(CO2PSheet(67+k,15:15+EndYear-2013-1),[1 3 2]);
end
...
function [dSij, counter] = get_dSji(t, MWKA, dSij, S, TLCOEg, dt, dTLCOE, Unc, i, k, action, counter)
