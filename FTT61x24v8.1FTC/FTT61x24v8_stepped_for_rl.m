...
function Out = FTT61x24v8_stepped_for_rl(CostSheet,HistoricalG,HistoricalE,CapacityFactors,CSCData,Unc,SubSheet,FiTSheet,RegSheet,DPSheet,CO2PSheet,MWKASheet,dt,NET,NWR,EndYear)
...
for t = 17:N
    step_through_year(t, hw, U, REG, MWKA, dSij, NWR, NET, Shat, S, TLCOEg, D, Gb, Shat2, G, CF, MC, dMC, DD, DT, NLB, dt, d, W, CO2, Costs, CarbP, CSCData, dLCOE, dTLCOE, B, Unc, b, P, REfficiency, CSCType, Svar, r, FiT, T, Gam, isInclT, CFLB3, SLB3, N);
end
if ishandle(hw)
    close(hw);
end
...
function P = step_through_year(t, hw, U, REG, MWKA, dSij, NWR, NET, Shat, S, TLCOEg, D, Gb, Shat2, G, CF, MC, dMC, DD, DT, NLB, dt, d, W, CO2, Costs, CarbP, CSCData, dLCOE, dTLCOE, B, Unc, b, P, REfficiency, CSCType, Svar, r, FiT, T, Gam, isInclT, CFLB3, SLB3, N);
    t
    if mod(t,4)==0
        if ~ishandle(hw)
            return
    ...
    S = shares_equation(MWKA, isReg, dSij, NWR, NET, t, Shat, S, TLCOEg, U, D, Gb, Shat2);
...
function S = shares_equation(MWKA, isReg, dSij, NWR, NET, t, Shat, S, TLCOEg, U, D, Gb, Shat2);
    counter=0;
    for k = 1:NWR
...
%                         dSij(j,i,k) = -dSij(i,j,k);
                        tlcoe_i = TLCOEg(i,k,t-1);
                        tlcoe_j = TLCOEg(j,k,t-1);
                        counter = counter + 1
%                         fprintf(counter)
                    end
                end
