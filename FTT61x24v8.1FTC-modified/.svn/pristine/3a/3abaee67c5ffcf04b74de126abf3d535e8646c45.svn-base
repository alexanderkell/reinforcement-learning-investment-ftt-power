...
if sum(G([17 18 19 22])) > 0
    %Capacity factor of renewables
    CFvar = (S(17)*CF(17)+S(18)*CF(18)+S(19)*CF(19)+S(22)*CF(22))/sum(S([17 18 19 22]));
else
    CFvar = 1;
end
%Capacity factor by load band
CFLB = [7500 4400 2200 700 80 CFvar*8766]/8766;
%Heights of load bands: normalised such that sum(GLB) = 1 (as defined in Uekerdt)
...
ULB(6) = GLB(6)/CFLB(6);
%Generation in each load band: normalised such that sum(GLB) = 1
GLB = ULB.*CFLB;
%Now normalise ULB such that ULB(6) = sum(S([17 18 19 22]))
%and sum(ULB) = 1, i.e. sum(ULB(1:5)) = non-renewables
CFavg = sum(CF.*S);
ULB(6) = GLB(6)/CFLB(6)*CFavg;
ULB(1:5) = ULB(1:5)/sum(ULB(1:5))*(1-sum(S([17 18 19 22])));
GLB;
end
